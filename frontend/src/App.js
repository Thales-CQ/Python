      ...
      <div className="sidebar-trigger">
        <div className="sidebar-content">
          ...
          <nav className="py-4 px-2 space-y-1 overflow-y-auto h-full pb-32">
            ...
            {/* Menu específico para vendas - moved outside the management conditional */}
            {user?.role === 'vendas' && (
              <>
                <div className="border-t-2 border-yellow-300 my-3 pt-3">
            ...
            {/* Menu de relatórios avançados para admin/gerente */}
            {hasPermission(['admin', 'manager']) && (
              <>
                <div className="border-t-2 border-yellow-300 my-3 pt-3">
                  <div className="text-xs font-bold text-yellow-100 px-3 mb-2">RELATÓRIOS AVANÇADOS</div>
                </div>

                <button
                  onClick={() => setCurrentPage('all-reports')}
                  className={`w-full flex items-center px-3 py-3 text-sm font-bold rounded-lg transition-all duration-200 menu-item-enter ${
                    currentPage === 'all-reports' 
                      ? 'bg-white text-red-600 shadow-lg transform scale-105' 
                      : 'text-white hover:bg-white hover:text-red-600 hover:shadow-md hover:transform hover:scale-105'
                  }`}
                >
...
const PerformancePage = ({ user, token }) => {
  ...
  const MonthlyChart = ({ vendedorData, title, dataKey }) => {
    ...
    return (
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <div className="space-y-4">
          {vendedorList.map(([vendedorId, data]) => (
            <div key={vendedorId} className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-gray-700">{data.vendedor_name}</span>
                <span className="text-sm text-gray-500">
                  {dataKey === 'monthly_data' ? `${data.total_sales} vendas` : `R$ ${data.total_value.toFixed(2)}`}
                </span>
              </div>
              <div className="flex space-x-1 h-8">
                {data[dataKey].map((value, monthIndex) => {
                  const height = maxValue > 0 ? (value / maxValue) * 100 : 0;
                  const colorClass = vendedorId === rankedVendedores[0]?.[0] ? 'bg-green-500' :
                                   vendedorId === rankedVendedores[1]?.[0] ? 'bg-blue-500' :
                                   vendedorId === rankedVendedores[2]?.[0] ? 'bg-yellow-500' : 'bg-gray-400';
                  
                  return (
                    <div
                      key={monthIndex}
                      className="flex-1 bg-gray-200 rounded-sm relative group cursor-pointer"
                      title={`${months[monthIndex]}: ${dataKey === 'monthly_data' ? value : `R$ ${value?.toFixed(2) || '0.00'}`}`}
                    >
                      <div
                        className={`${colorClass} rounded-sm transition-all duration-300 hover:opacity-80`}
  ...
  const RankingCard = ({ rank, vendedorName, sales, value, isTopPerformer }) => (
    <div className={`bg-white shadow rounded-lg p-6 ${isTopPerformer ? 'ring-2 ring-yellow-400' : ''}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold ${
            rank === 1 ? 'bg-yellow-500' : rank === 2 ? 'bg-gray-400' : rank === 3 ? 'bg-orange-600' : 'bg-gray-300'
          }`}>
            {rank}
          </div>
          <div className="ml-3">
            <p className="text-sm font-medium text-gray-900">{vendedorName}</p>
            <p className="text-xs text-gray-500">
              {rank === 1 ? '🏆 TOP PERFORMER' : rank === 2 ? '🥈 2º LUGAR' : rank === 3 ? '🥉 3º LUGAR' : 'PARTICIPANTE'}
            </p>
          </div>
        </div>
        {isTopPerformer && (
      ...
      <div className="mt-4">
        <div className="flex justify-between">
          ...
          <span className="text-sm font-bold text-gray-900">{sales}</span>
        </div>
        <div className="flex justify-between mt-1">
          <span className="text-sm text-gray-500">Valor:</span>
          <span className="text-sm font-bold text-green-600">R$ {value.toFixed(2)}</span>
        </div>
        <div className="flex justify-between mt-1">
          <span className="text-sm text-gray-500">Ticket Médio:</span>
          <span className="text-sm font-bold text-blue-600">R$ {(value / sales).toFixed(2)}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-900">Dashboard de Performance & Gráficos</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 shadow rounded-lg p-6 text-white">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium opacity-80">Total Vendas</p>
              <p className="text-2xl font-bold">{totalSalesAllVendors}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-green-500 to-green-600 shadow rounded-lg p-6 text-white">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm font-medium opacity-80">Faturamento Total</p>
              <p className="text-2xl font-bold">R$ {totalValueAllVendors.toFixed(2)}</p>
            </div>
          </div>
        </div>

        <div className="bg-gradient-to-r from-purple-500 to-purple-600 shadow rounded-lg p-6 text-white">
          <div className="flex items-center">
            <div className="flex-shrink-0">
              <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold">{vendedorList.length}</p>
            </div>
          </div>
        </div>

        <div className="bg-gradient-to-r from-orange-500 to-orange-600 shadow rounded-lg p-6 text-white">
          <div className="flex items-center">
            <div className="flex-shrink-0">
              <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
            ...
            <div className="ml-4">
              <p className="text-sm font-medium opacity-80">Ticket Médio</p>
              <p className="text-2xl font-bold">
                R$ {totalSalesAllVendors > 0 ? (totalValueAllVendors / totalSalesAllVendors).toFixed(2) : '0.00'}
              </p>
            </div>
          </div>
        </div>
      ...
      <div>
        <h2 className="text-xl font-bold text-gray-900 mb-4">🏆 Ranking de Performance {year}</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {rankedVendedores.slice(0, 3).map(([vendedorId, data], index) => (
            <RankingCard
              key={vendedorId}
              rank={index + 1}
              vendedorName={data.vendedor_name}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <MonthlyChart 
          vendedorData={performance} 
          title="📈 Vendas por Mês - Comparativo"
          dataKey="monthly_data"
        />
        <MonthlyChart 
          vendedorData={performance} 
          title="💰 Faturamento por Mês - Comparativo"
          dataKey="monthly_values"
        />
      </div>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-medium text-gray-900">📊 Tabela Detalhada de Performance</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {rankedVendedores.map(([vendedorId, data], index) => {
                const bestMonthIndex = data.monthly_data.indexOf(Math.max(...data.monthly_data));
                const bestMonthSales = data.monthly_data[bestMonthIndex];
                const performancePercentage = totalSalesAllVendors > 0 ? (data.total_sales / totalSalesAllVendors * 100) : 0;
                
                return (
                  <tr key={vendedorId} className={index < 3 ? 'bg-yellow-50' : ''}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      R$ {(data.total_value / data.total_sales).toFixed(2)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {months[bestMonthIndex]} ({bestMonthSales} vendas)
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
      ...
      {/* Monthly Breakdown */}
      <div className="bg-white shadow rounded-lg p-6">
        <h2 className="text-lg font-medium text-gray-900 mb-4">📅 Breakdown Mensal {year}</h2>
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead>
              <tr className="border-b">
                <th className="text-left py-2 px-3 text-sm font-medium text-gray-700">Vendedor</th>
                {months.map(month => (
                  <th key={month} className="text-center py-2 px-2 text-xs font-medium text-gray-500">
                    {month}
                  </th>
                ))}
            ...
            <tbody>
              {vendedorList.map(([vendedorId, data]) => (
                <tr key={vendedorId} className="border-b">
                  <td className="py-2 px-3 text-sm font-medium text-gray-900">
                    {data.vendedor_name}
                  </td>
                  {data.monthly_data.map((sales, monthIndex) => (
                    <td key={monthIndex} className="text-center py-2 px-2 text-xs">
                      <span className={`inline-block px-2 py-1 rounded ${
                        sales > 0 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'
                      }`}>
                        {sales}
                      </span>
                    </td>
                  ))}
                  <td className="text-right py-2 px-3 text-sm font-bold text-gray-900">
                    {data.total_sales}
                  </td>
                </tr>
              ))}
